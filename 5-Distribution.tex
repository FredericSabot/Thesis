...
A large share of DERs consist of solar and wind generation, \ie inverter-based generation. DERs are thus often represented with generic models as shown in Figure~\ref{fig:IBG}. These models represent (grid following) inverter-based generation as current-controlled sources with the following control blocks.
...
It should be noted that, \cite{ChaspierreThesis} developed a methodology to update the parameters of the load model when operating conditions change (\eg if solar irradiation increase, the behaviour of the load will change as well as its parameters). However, for the sake of simplicity, load models were always parametrised from scratch in the test cases in this chapter.
...
\begin{axis}[
    ...
    \foreach \n in {0,...,49}{
        \addplot+ [mark=none, color=black, dotted, opacity=0.5, line width=0.6pt] table [x=t,y=omega] {Figs/ISGT_frequency_plot/omega_\n.csv};
    }
    \addplot+ [mark=none, line width=1pt, blue] table [x=time,y=OMEGA_REF_omegaRef_grp_0_value] {Figs/ISGT_frequency_plot/5_equiv.csv};
    \addplot+ [mark=none, line width=1pt, red] table [x=time,y=OMEGA_REF_omegaRef_grp_0_value] {Figs/ISGT_frequency_plot/95_equiv.csv};

    % UFLS 95 (trip, not activation)
    \node at (axis cs:11.21750,0.979421) [circle, fill, inner sep=1.5pt] {};

    % UFLS 5
    \node at (axis cs:9.918750,0.977498) [circle, fill, inner sep=1.5pt] {};
    \node at (axis cs:9.998750,0.976133) [circle, fill, inner sep=1.5pt] {};
    \node at (axis cs:10.008750,0.974791) [circle, fill, inner sep=1.5pt] {};
    \node at (axis cs:9.598750,0.976133) [anchor=east, align=left] {UFLS trips};
