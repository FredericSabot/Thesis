...
d^{\infty}_{jk} = \max{
\begin{bmatrix}
  \max_g{\left(\abs{P_{g, j} - P_{g, k}}, \abs{Q_{g, j} - Q_{g, k}}\right)}\\
  \max_b{\left(\abs{P_{b, j} - P_{b, k}}, \abs{Q_{b, j} - Q_{b, k}}\right)}
\end{bmatrix}
}
% \end{split}
\end{equation}
...
\begin{minipage}[c]{0.42\linewidth}
  \strut\vspace*{-\baselineskip}\newline % Minipage vertical allignement black magic
  \centering
  \includegraphics[width=\linewidth]{Figs/RTS.png}
...
\end{tabular}
\end{adjustbox}
\caption{Additional accuracy brought by the use of a second feature}
...
\label{fig:value_N1}
\begin{tikzpicture}
\pgfplotsset{width=0.95\linewidth}
\begin{groupplot}[
  group style={
    group name=my plots,
    group size=1 by 2,
...
\nextgroupplot[align=center, ylabel={Share of operating\\conditions [\%]}]
